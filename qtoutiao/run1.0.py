...
def is_frist_page():  # 判断是否首页 返回一个元组  就是点击的坐标
    # 得到一个元组 (0, 0, 0, 255)
    a = get_pixel_colour(r'C:\Users\machunpo\Desktop\myimages\funtoutiao.png', 115, 92)
...
def let_us_go(a=1):
    for i in range(loop_time_news):
        ...
        speak_and_print('{}共{}次，{}结束第{}次'.format(
            a, loop_time_news, chengong_or_shibai, i + 1))
        time.sleep(2)
        os.system('adb shell input keyevent BACK')
        time.sleep(1)
...
def wode():
    time.sleep(5)
    os.system('adb shell input tap 648 1216')
    time.sleep(25)
    os.system('adb shell input tap 72 1216')
    time.sleep(5)
